...
print(
    f"Value counts of the STATUS column: {credit_df['STATUS'].value_counts()}")

# Print the total observations in the credit_df
print(
    f"Total observations in the credit_df: {credit_df[credit_df['STATUS'].isin(['C', '0', '1'])]['ID'].value_counts()}")
...
status_fractions.columns = [
    f'{col}_fraction' for col in status_fractions.columns]
...
app_df.to_csv('credit_delinquency_v2.csv')
